...
export async function showInputDialog(
  ...
  try {
    // Tauri dialog 插件目前不直接支持输入对话框
    // 使用浏览器原生的 prompt 作为临时方案
    // TODO: 后续可考虑使用自定义 Modal 组件替代
    const result = window.prompt(message, defaultValue || '');
    return result;
  } catch (e) {
    console.error('显示输入对话框失败:', e);
    throw e;
  }
}
